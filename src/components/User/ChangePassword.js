...
const ChangePassword = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (actualData.oldpass && actualData.password && actualData.password_confirmation) {
      if (actualData.password === actualData.password_confirmation) {
        //console.log(actualData);
        fetch('http://localhost:43608/api/Customers/'+user.customer_Id , 
        {
        method: 'PUT',
        headers: {
  ...
  return <>
    <Box sx={{ display: 'flex', flexDirection: 'column', flexWrap: 'wrap', maxWidth: 600, mx: 4 }}>
      ...
      <TextField margin="normal" required fullWidth name="oldpass" label="Old Password" type="password" id="oldpass" />
